...
const ServiceDetail = () => {
  ...
  if (!currentService?.details || !currentService?.category) return null;

  const details = currentService.details;
  const category = currentService.category;
  const categoryConfig = categories.find(cat => cat._id === category._id)?.config;

  if (!categoryConfig?.fields) return null;
  ...
  const renderField = (field) => {
    ...
    if (field.type === 'boolean') {
      return (
        <div className="flex items-center">
          <CheckCircle className={`w-5 h-5 mr-2 ${value ? 'text-green-500' : 'text-gray-300'}`} />
    ...
    if (field.type === 'select') {
      const selectedValue = Array.isArray(value) ? 
        value.map(v => field.options.find(o => o === v)).join(', ') :
        field.options.find(o => o === value);
      
      return (
        <div>
          <h4 className="text-sm font-medium text-gray-500">{field.label}</h4>
          <p className="text-gray-900">{selectedValue || 'Not specified'}</p>
        </div>
      );
    }
  ...
  // Add FAQ section if there are any FAQs
  if (allFaqs.length > 0) {
  ...
  return sectionComponents.map((section, index) => (
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                      ...
                      <div className="bg-white rounded-2xl shadow-lg p-8 border border-gray-100">
                          <div className="flex flex-col md:flex-row md:items-start md:justify-between mb-6">
                              <div className="flex-1">
                                  ...
                                  <div className="flex items-center space-x-6 mb-4">
                                      <div className="flex items-center">
                                          ...
                                          <span className="text-sm text-gray-600">
                                              Vendor: {currentService?.vendor?.name}
                                          </span>
                                      </div>
                                      <div className="flex items-center">
                                  ...
                                  {currentService?.variants?.length > 0 && (
                                      <div className="mb-6">
                                              ...
                                              {currentService.variants.map((variant) => (
                                                  ...
                                                  >
                                                      ...
                                                      <p className="text-xs text-gray-500">
                                                          {variant.unit
                                                              ? `Charged per ${variant.unit}`
                                                              : "Fixed charge (optional add-on)"}
                                                      </p>
                                                  </div>
                                              ))}
                                          </div>
                              ...
                              <div className="mb-6 p-4 bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg border border-blue-100 flex items-center">
                                  ...
                                  <div>
                                      ...
                                      <p className="text-sm text-gray-600">
                                          Valid until{" "}
                                          {new Date(
                                              currentService?.offer.validTill
                                          ).toLocaleDateString()}
                                      </p>
                                  </div>
                              </div>
                ...
                <div className="text-sm text-gray-600">Starting price</div>
                          ...
                          <button
                              onClick={() => navigate(`/booking/${currentService?._id}`)}
                          ...
                          <div className="space-y-4">
                              ...
                              <div className="flex items-center space-x-3">
                                  ...
                                  <div>
                                      ...
                                      <div className="font-medium text-gray-900">
                                          {currentService?.vendor?.email || "Not provided"}
                                      </div>
                                  </div>
                              </div>
                              <div className="flex items-center space-x-3">
