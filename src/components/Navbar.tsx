...
import { Calendar, Gift, HelpCircle, Home, Menu, User, X, ChevronDown } from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                      ...
                      <div key={index} className="space-y-3">
                        ...
                        <ul className="space-y-2">
                          {category.subcategories.map((subcategory, subIndex) => (
                            <li key={subIndex}>
                              <Link
                                to={`/vendor/${category.name.toLowerCase().replace(/\s+/g, '-')}/${subcategory.toLowerCase().replace(/\s+/g, '-')}`}
                        ...
                        <Link
                          to={`/vendor/${category.name.toLowerCase().replace(/\s+/g, '-')}`}
        ...
        {isMenuOpen && (
          <div className="md:hidden mt-4 space-y-4">
            <Link to="/" className="block hover:text-purple-200 transition-colors flex items-center">
              <Home className="w-4 h-4 mr-2" /> HOME
            </Link>
            <Link to="/bookings" className="block hover:text-purple-200 transition-colors flex items-center">
              <Calendar className="w-4 h-4 mr-2" /> BOOKINGS
            </Link>
            <Link to="/offer" className="block hover:text-purple-200 transition-colors flex items-center">
            ...
            <Link to="/help" className="block hover:text-purple-200 transition-colors flex items-center">
